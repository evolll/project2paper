...
- `agent-workspace/project-input/config.json` — `length`, `output_format`, `novelty_highlight`, `latex_template`, `references`, `base_path`
- `agent-workspace/agent_helpers.py` — use `render_novelty_marker()`, `get_chapter_list()`, `generate_main_tex()`, `render_placeholder_block()`

## Output

### LaTeX format
Write to `agent-workspace/output/`:
- `main.tex` — Root file with `\include{}` commands (use `agent_helpers.generate_main_tex()`)
...
## Template selection (LaTeX only)
Check `config.json` → `latex_template`:
- `article` → Standard article, general purpose
- `ieee` → IEEE conference format
- `acm` → ACM conference format

The preamble and document class are auto-generated by `agent_helpers.generate_main_tex()`.
Each chapter file follows the structure in `agent-workspace/templates/latex/{template}-chapter.tex`.
