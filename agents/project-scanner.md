...
```json
{
  ...
  "total_files": 123,
  "total_lines": 45678,
  "structure": { "src/": { "type": "dir", "children": [...] } },
...
4. Detect the primary language and framework from file extensions and config files
5. Identify entry points, test files, config files
6. Count total files and lines of code per language
7. Write the output JSON

Be thorough but fast. Focus on structure, not content. Use `find` and basic file listing to build the map.
